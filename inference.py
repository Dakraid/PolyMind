...
API_KEY = Config.values.backend_config.api_key
TABBY = True if Config.values.backend == "tabbyapi" else False
if TABBY:
...
def infer(
    ...
    temperature=0.7,
    username="",
    bsysep=Config.values.llm_parameters.sys_begin_sep,
    esysep=Config.values.llm_parameters.sys_end_sep,
    modelname="",
    eos="</s><s>",
    beginsep=Config.values.llm_parameters.begin_sep,
    endsep=Config.values.llm_parameters.end_sep,
    mem=[],
    few_shot="",
    max_tokens=250,
    stopstrings=[],
    top_p=1.0,
    top_k=Config.values.llm_parameters.top_k,
    min_p=0.0,
    streamresp=False,
    reppenalty=Config.values.llm_parameters.repetition_penalty,
    max_temp=Config.values.llm_parameters.max_temp,
    min_temp=Config.values.llm_parameters.min_temp
):
    content = ""
    memory = mem
    prompt = (
    ...
    removal = 0
    while (
        tokenize(prompt)["length"] + max_tokens / 2 > Config.values.ctxlen
        and len(memory) > 2
    ):
    ...
    stopstrings += ["</s>", "<</SYS>>", "[Inst]", "[/INST]", Config.values.llm_parameters.sys_begin_sep, Config.values.llm_parameters.sys_end_sep, Config.values.llm_parameters.begin_sep, Config.values.llm_parameters.end_sep]
    payload = {
        "prompt": prompt,
